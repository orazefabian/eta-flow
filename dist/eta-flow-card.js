...
`;var Ht,Nt;!function(t){t.language="language",t.system="system",t.comma_decimal="comma_decimal",t.decimal_comma="decimal_comma",t.space_comma="space_comma",t.none="none"}(Ht||(Ht={})),function(t){t.language="language",t.system="system",t.am_pm="12",t.twenty_four="24"}(Nt||(Nt={}));let It=class extends lt{setConfig(t){this._config={nodes:{},...t}}render(){return this._config&&this.hass?V`
  ...
  `,t([ft({attribute:!1})],It.prototype,"hass",void 0),t([mt()],It.prototype,"_config",void 0),It=t([dt("eta-flow-card-editor")],It),console.info("%c ETA-FLOW-CARD %c v0.1.3 ","color: #fff; background: #4caf50; font-weight: 700;","color: #4caf50; background: #1c1c1c;"),window.customCards=window.customCards||[],window.customCards.push({type:_t,name:"ETA Flow Card",description:"Animated heat-flow visualization for ETA heating systems (pellet, log & solar).",preview:!0,documentationURL:"https://github.com/orazefabian/eta-flow"});const jt=(t,e,i)=>Math.max(e,Math.min(i,t));function Lt(t,e,i,s,r=4){const n=e.x-t.x,o=e.y-t.y,a=Math.hypot(n,o)||1,c=n/a,l=o/a;return{x1:t.x+c*(i+r),y1:t.y+l*(i+r),x2:e.x-c*(s+r),y2:e.y-l*(s+r)}}let Dt=class extends lt{static async getConfigElement(){return document.createElement("eta-flow-card-editor")}static getStubConfig(){return{type:`custom:${_t}`,title:"Heizung",nodes:{puffer:{},solar:{},kessel:{},warmwasser:{},heizkreis:{},aussen:{}}}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={nodes:{},edges:{},...t}}getCardSize(){return 6}render(){if(!this._config||!this.hass)return K;const t=this._nodeIds().filter(t=>this._nodeVisible(t)),e=this._resolvedEdges().filter(e=>t.includes(e.from)&&t.includes(e.to)),i=(this._config.control_links??Et).filter(e=>t.includes(e.from)&&t.includes(e.to)),s=[];for(const e of t){const t="circle"===this._nodeKind(e)?this._circleIconSpec(e):this._badgeIconSpec(e);t&&s.push(t)}for(const t of e){const e=this._pumpIconSpec(t);e&&s.push(e)}return V`
      <ha-card>
        ...
        <div class="icon-layer" xmlns="http://www.w3.org/1999/xhtml">
          ${t.map(t=>V`
              <ha-icon
                ...
                icon=${t.icon}
                style=${`left:${t.cx}px;top:${t.cy}px;--mdc-icon-size:${t.size}px;`}
              ></ha-icon>
            `)}
        </div>
      </foreignObject>
    `:K}_circleIconSpec(t){const e=this._geom(t);if(!e)return;const i=Ut(this._cfg(t),this.hass),s=this._nodeRadius(t),r=!!i.state||!!i.secondary,n=jt(Math.round(.62*s),14,40),o=r?e.y-.42*s:e.y-.3*s;return{icon:this._nodeIcon(t),cx:e.x,cy:o,size:n,cls:""}}_badgeIconSpec(t){const e=this._geom(t);if(!e)return;const i=this._nodeRadius(t),s=jt(Math.round(.66*i),14,28);return{icon:this._nodeIcon(t),cx:e.x,cy:e.y-.44*i,size:s,cls:""}}_pumpIconSpec(t){const e=this._edgePump(t.key);if(!e?.entity)return;const i=this._geom(t.from),s=this._geom(t.to);if(!i||!s)return;const r=Ot(this.hass,e.entity,e.active_states),n=Math.round(.95*vt);return{icon:e.icon??yt,cx:(i.x+s.x)/2,cy:(i.y+s.y)/2,size:n,cls:("pump "+(r?"on":"")).trim()}}_nodeVisible(t){const e=this._cfg(t);return!e?.hidden&&(!!this._geom(t)&&(this._hasData(e)||"puffer"===t))}_resolvedEdges(){const t=new Map(kt.map(t=>[t.key,t])),e=new Set([...t.keys(),...Object.keys(this._config.edges??{})]),i=[];for(const s of e){const e=this._config.edges?.[s],r=t.get(s),n=e?.from??r?.from,o=e?.to??r?.to;n&&o&&i.push({key:s,from:n,to:o})}return i}_renderEdge(t){const e=this._geom(t.from),i=this._geom(t.to);if(!e||!i)return K;const{x1:s,y1:r,x2:n,y2:o}=Lt(e,i,this._nodeRadius(t.from),this._nodeRadius(t.to)),a=`edge-${t.key}`,c=`M ${s} ${r} L ${n} ${o}`,l=this._config.edges?.[t.key],h=Pt(l,this.hass),d=this._nodeColor(h.reverse?t.to:t.from),p=l?.show_label??this._config.show_edge_labels??!1?function(t,e){if(!t)return;if(t.label_entity)return Rt(e,t.label_entity);const i=Mt(t,e);if("power"===i)return Rt(e,t.entity);if("delta"===i){const i=St(e,t.from_entity),s=St(e,t.to_entity);if(void 0===i||void 0===s)return;return`Δ${Math.round(10*(i-s))/10}°`}}(l,this.hass):void 0;let u=K;if(p){const s=(e.x+i.x)/2,r=(e.y+i.y)/2,n=Math.hypot(i.x-e.x,i.y-e.y)||1,o=-(i.y-e.y)/n,a=(i.x-e.x)/n,c=this._edgePump(t.key)?22:11;u=W`<text class="edge-label" x=${s+o*c} y=${r+a*c}
      ...
      ${u}
      ${f?W`<path class="edge-hit" d=${c} @click=${()=>this._openMoreInfo(f)}></path>`:K}
    `}_renderDots(t,e,i,s){const r=i?"1;0":"0;1";return W`${Array.from({length:3},(i,n)=>W`
      ...
      <g style=${`color:${a}`} class="clickable" @click=${()=>this._openMoreInfo(e.entity)}>
        <circle
          ...
          stroke="currentColor"
        ></circle>
        ${e.hide_label?K:W`<text class="pump-label" x=${r+c+5} y=${n} dominant-baseline="central">${l}</text>`}
      </g>
    `}_renderNode(t,e){return"circle"===this._nodeKind(t)?this._renderCircle(t,e):this._renderBadge(t)}_renderCircle(t,e){const i=this._geom(t);if(!i)return K;const s=this._cfg(t),r=Ut(s,this.hass),n=this._nodeColor(t),o=this._nodeRadius(t),a=this._nodeActive(t,e),c=!!r.state,l=!c&&!!r.secondary,h=c||l,d=jt(.36*o,12,22).toFixed(1),p=jt(.28*o,10,16).toFixed(1),u=jt(.3*o,11,16).toFixed(1),f=h?i.y+.04*o:i.y+.36*o,m=i.y+.44*o,_=i.y+o+.8*Number(u)+3,$=!(!s?.level&&!s?.layers?.length),g=this._nodeEntity(t);return W`
      <g
        style=${`color:${n}`}
        class=${g?"clickable":""}
      ...
      >
        ...
        ${r.primary?W`<text
              ...
              >${r.primary}</text>`:K}
        ${c?this._renderPill(i.x,m,o,r.state):K}
        ${l?W`<text
    ...
    `}_renderBadge(t){const e=this._geom(t);if(!e)return K;const i=this._cfg(t),s=Ut(i,this.hass),r=this._nodeColor(t),n=this._nodeRadius(t),o="gauge"===this._nodeKind(t)||!0===i?.gauge?function(t,e){const i=St(e,t?.primary);if(void 0===i)return;const s=t?.min??0,r=t?.max??100;return r<=s?void 0:Math.max(0,Math.min(1,(i-s)/(r-s)))}(i,this.hass):void 0,a=e.y+.2*n,c=this._nodeEntity(t);return W`
      ...
      >
        <circle
          ...
          stroke-width=${this._nodeStroke(t)}
        ></circle>
        ${s.primary?W`<text class="badge-text" x=${e.x} y=${a} dominant-baseline="central">${s.primary}</text>`:K}
        ${void 0!==o?W`
