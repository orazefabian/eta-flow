...
`;var Nt,Tt;!function(t){t.language="language",t.system="system",t.comma_decimal="comma_decimal",t.decimal_comma="decimal_comma",t.space_comma="space_comma",t.none="none"}(Nt||(Nt={})),function(t){t.language="language",t.system="system",t.am_pm="12",t.twenty_four="24"}(Tt||(Tt={}));let Ht=class extends ct{setConfig(t){this._config={nodes:{},...t}}render(){return this._config&&this.hass?V`
      <div class="card-config">
        ...
        ${Object.values(_t).map(t=>this._renderNodeRow(t.id,t.label))}
  ...
  `,t([ft({attribute:!1})],Ht.prototype,"hass",void 0),t([$t()],Ht.prototype,"_config",void 0),Ht=t([dt("eta-flow-card-editor")],Ht),console.info("%c ETA-FLOW-CARD %c v0.1.0 ","color: #fff; background: #4caf50; font-weight: 700;","color: #4caf50; background: #1c1c1c;"),window.customCards=window.customCards||[],window.customCards.push({type:mt,name:"ETA Flow Card",description:"Animated heat-flow visualization for ETA heating systems (pellet, log & solar).",preview:!0,documentationURL:"https://github.com/orazefabian/eta-flow"});const jt=(t,e,i)=>Math.max(e,Math.min(i,t));function Lt(t,e,i,s,r=4){const n=e.x-t.x,o=e.y-t.y,a=Math.hypot(n,o)||1,l=n/a,c=o/a;return{x1:t.x+l*(i+r),y1:t.y+c*(i+r),x2:e.x-l*(s+r),y2:e.y-c*(s+r)}}let Dt=class extends ct{static async getConfigElement(){return document.createElement("eta-flow-card-editor")}static getStubConfig(){return{type:`custom:${mt}`,title:"Heizung",nodes:{puffer:{},solar:{},kessel:{},warmwasser:{},heizkreis:{},aussen:{}}}}setConfig(t){if(!t)throw new Error("Invalid configuration");this._config={nodes:{},edges:{},...t}}getCardSize(){return 6}render(){if(!this._config||!this.hass)return K;const t=this._nodeIds().filter(t=>this._nodeVisible(t)),e=this._resolvedEdges().filter(e=>t.includes(e.from)&&t.includes(e.to)),i=(this._config.control_links??kt).filter(e=>t.includes(e.from)&&t.includes(e.to));return V`
      <ha-card>
    ...
    `}_nodeIds(){const t=Object.keys(_t);for(const e of Object.keys(this._config.nodes??{}))t.includes(e)||t.push(e);return t}_cfg(t){return this._config.nodes?.[t]}_geom(t){const e=this._cfg(t),i=_t[t],s=e?.x??i?.x,r=e?.y??i?.y;if(void 0!==s&&void 0!==r)return{x:s,y:r}}_nodeKind(t){return this._cfg(t)?.kind??_t[t]?.kind??wt}_nodeColor(t){return this._cfg(t)?.color??_t[t]?.color??xt}_nodeRadius(t){return this._cfg(t)?.radius??_t[t]?.radius??bt}_nodeStroke(t){return this._cfg(t)?.stroke_width??2.5}_nodeIcon(t){return this._cfg(t)?.icon??_t[t]?.icon??At}_nodeLabel(t){return this._cfg(t)?.name??_t[t]?.label??t}_hasData(t){return!!(t?.primary||t?.level||t?.layers?.length)}_nodeVisible(t){const e=this._cfg(t);return!e?.hidden&&(!!this._geom(t)&&(this._hasData(e)||"puffer"===t))}_resolvedEdges(){const t=new Map(Et.map(t=>[t.key,t])),e=new Set([...t.keys(),...Object.keys(this._config.edges??{})]),i=[];for(const s of e){const e=this._config.edges?.[s],r=t.get(s),n=e?.from??r?.from,o=e?.to??r?.to;n&&o&&i.push({key:s,from:n,to:o})}return i}_renderEdge(t){const e=this._geom(t.from),i=this._geom(t.to);if(!e||!i)return K;const{x1:s,y1:r,x2:n,y2:o}=Lt(e,i,this._nodeRadius(t.from),this._nodeRadius(t.to)),a=`edge-${t.key}`,l=`M ${s} ${r} L ${n} ${o}`,c=this._config.edges?.[t.key],h=Mt(c,this.hass),d=this._nodeColor(h.reverse?t.to:t.from),p=c?.show_label??this._config.show_edge_labels??!1?function(t,e){if(!t)return;if(t.label_entity)return zt(e,t.label_entity);const i=Pt(t,e);if("power"===i)return zt(e,t.entity);if("delta"===i){const i=Ct(e,t.from_entity),s=Ct(e,t.to_entity);if(void 0===i||void 0===s)return;return`Δ${Math.round(10*(i-s))/10}°`}}(c,this.hass):void 0;let u=K;if(p){const s=(e.x+i.x)/2,r=(e.y+i.y)/2,n=Math.hypot(i.x-e.x,i.y-e.y)||1,o=-(i.y-e.y)/n,a=(i.x-e.x)/n,l=this._edgePump(t.key)?22:11;u=W`<text class="edge-label" x=${s+o*l} y=${r+a*l}
        dominant-baseline="central">${p}</text>`}return W`
    ...
    `}_renderDots(t,e,i,s){const r=i?"1;0":"0;1";return W`${Array.from({length:3},(i,n)=>W`
      ...
      <g style=${`color:${a}`}>
        ...
        >
          <ha-icon
            icon=${e.icon??yt}
            style=${`color: var(--eta-text); --mdc-icon-size: ${c}px; width:${c}px; height:${c}px;`}
          ></ha-icon>
        </foreignObject>
        ${e.hide_label?K:W`<text class="pump-label" x=${r+l+5} y=${n} dominant-baseline="central">${h}</text>`}
      </g>
    `}_renderNode(t,e){return"circle"===this._nodeKind(t)?this._renderCircle(t,e):this._renderBadge(t)}_renderCircle(t,e){const i=this._geom(t);if(!i)return K;const s=this._cfg(t),r=Rt(s,this.hass),n=this._nodeColor(t),o=this._nodeRadius(t),a=this._nodeActive(t,e),l=!!r.state,c=!l&&!!r.secondary,h=l||c,d=jt(Math.round(.62*o),14,40),p=jt(.36*o,12,22).toFixed(1),u=jt(.28*o,10,16).toFixed(1),f=jt(.3*o,11,16).toFixed(1),$=h?i.y-.42*o:i.y-.3*o,m=h?i.y+.04*o:i.y+.36*o,_=i.y+.44*o,g=i.y+o+.8*Number(f)+3,y=!(!s?.level&&!s?.layers?.length);return W`
      <g style=${`color:${n}`}>
        <circle
        ...
        >
          <ha-icon
            icon=${this._nodeIcon(t)}
            style=${`color: var(--eta-text); --mdc-icon-size: ${d}px; width:${d}px; height:${d}px;`}
          ></ha-icon>
        </foreignObject>
        ${r.primary?W`<text
                class="node-primary"
              ...
              >${r.primary}</text>`:K}
        ${l?this._renderPill(i.x,_,o,r.state):K}
        ${c?W`<text
    ...
    `}_renderBadge(t){const e=this._geom(t);if(!e)return K;const i=this._cfg(t),s=Rt(i,this.hass),r=this._nodeColor(t),n=this._nodeRadius(t),o=jt(Math.round(.52*n),12,26),a="gauge"===this._nodeKind(t)||!0===i?.gauge?function(t,e){const i=Ct(e,t?.primary);if(void 0===i)return;const s=t?.min??0,r=t?.max??100;return r<=s?void 0:Math.max(0,Math.min(1,(i-s)/(r-s)))}(i,this.hass):void 0,l=e.y+.2*n;return W`
      <g style=${`color:${r}`}>
        <circle
          class="badge"
          cx=${e.x}
        ...
        >
          <ha-icon
            icon=${this._nodeIcon(t)}
            style=${`color: var(--eta-text); --mdc-icon-size: ${o}px; width:${o}px; height:${o}px;`}
          ></ha-icon>
        </foreignObject>
    ...
    `}_nodeActive(t,e){return e.filter(e=>e.from===t||e.to===t).some(t=>Mt(this._config.edges?.[t.key],this.hass).active)}};Dt.styles=Ut,t([ft({attribute:!1})],Dt.prototype,"hass",void 0),t([$t()],Dt.prototype,"_config",void 0),Dt=t([dt(mt)],Dt);export{Dt as EtaFlowCard};
